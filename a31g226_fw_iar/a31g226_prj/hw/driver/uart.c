...
bool uartOpen(uint8_t ch, uint32_t baud)
{
  ...
  switch(ch)
  {
    ...
    case _DEF_UART2:    //SY7t611
      uart_tbl[ch].type    = UART_HW_TYPE_MCU;
      uart_tbl[ch].baud    = baud;

      uart_tbl[ch].p_husart = USART13;
      ...
      NVIC_SetPriority(USART12_13_SPI21_IRQn, 5);
    ...
    case _DEF_UART3:        //debug
      ...
      HAL_UART_Init(uart_tbl[ch].p_huart, &uart_tbl[ch].uart_init);
    ...
    case _DEF_UART4:
      ...
      HAL_GPIO_ConfigPullup(PB, 7, PCU_PUPD_PULL_UP);
...
{
  uart_tbl_t *p_huart = &uart_tbl[_DEF_UART3];
  uint32_t status_reg;
  uint32_t isr_id;

  status_reg = p_huart->p_huart->IER;
  isr_id = status_reg & 0x01;

  if(isr_id & UART_IER_DRIE_Msk)
  {
    uint8_t read_data;
    read_data = p_huart->p_huart->RBR;

    qbufferWrite(&p_huart->qbuffer, &read_data, 1);
...
{
  uart_tbl_t *p_huart = &uart_tbl[_DEF_UART4];
  uint32_t status_reg;
  uint32_t isr_id;
...
{
  uart_tbl_t *p_usart = &uart_tbl[_DEF_UART2];
